...
import { Bot, LayoutGrid, StickyNote, Users } from 'lucide-vue-next';
...
const composerUrl = dashboardRoutes.composer.url();
</script>
    ...
    <template v-if="variant === 'sheet'">
        <nav class="-mx-3 space-y-1">
            ...
            <Link
                :href="composerUrl"
                class="flex items-center gap-x-3 rounded-lg px-3 py-2 text-sm font-medium hover:bg-accent"
                :class="whenCurrentUrl(composerUrl, activeItemStyles)"
            >
                <Bot class="h-5 w-5" />
                Композитор
            </Link>
    ...
    <template v-else>
        ...
        <NavigationMenuItem class="relative flex h-full items-center">
            <Link
                :class="[
                    navigationMenuTriggerStyle(),
                    whenCurrentUrl(composerUrl, activeItemStyles),
                    'h-9 cursor-pointer px-3',
                ]"
                :href="composerUrl"
            >
