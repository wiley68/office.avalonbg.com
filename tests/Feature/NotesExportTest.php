...
test('guest is redirected when starting manual export', function () {
    post(route('dashboard.notes.export'))->assertRedirect();
});

test('manual export returns download url and file downloads once for owner', function () {
    $user = User::factory()->create();
    Note::factory()->for($user)->count(2)->create();

    actingAs($user);

    $response = post(route('dashboard.notes.export'));
    $response->assertOk();
...
test('export decrypts encrypted note body in xlsx', function () {
    ...
    $result = post(route('dashboard.notes.export'))->assertOk()->json();
